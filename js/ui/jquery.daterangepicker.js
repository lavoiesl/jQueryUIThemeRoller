...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			//{text: '本周', dateStart:  'monday', dateEnd: 'today' },
                        {text: '上周', dateStart: function(){ return Date.parse('last monday').moveToDayOfWeek(1,-1) }, dateEnd: function(){ return Date.parse('last monday').moveToDayOfWeek(0, -1) } },
        ...
        var selectCurrentWeek = function() {
            window.setTimeout(function () {
                rp.find('.ui-datepicker-current-day a').addClass('ui-state-active');
            }, 1);
        }
	...
	var datepickerOptions = {
		...
		onSelect: function() {
                    var weekDay = null;
                    var specificMonth = null;
                    var month = null;
                    var year = null;
                    if(rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active')){
                        rp.find('.range-end').datepicker('setDate', rp.find('.range-start').datepicker('getDate') );
                    }
                    if(rp.find('.ui-daterangepicker-specificWeek').is('.ui-state-active')){
                        weekDay = rp.find('.week-picker').datepicker('getDate');
                    }
                    if(rp.find('.ui-daterangepicker-specificMonth').is('.ui-state-active')){
                        month = rp.find(".month-picker .ui-datepicker-month :selected").val();
                        year = rp.find(".month-picker .ui-datepicker-year :selected").val();
                        specificMonth = true;
                    }
                    ...
                    if(rangeInput.length == 2){
                        ...
                        if (weekDay) {
                            startDate = new Date(weekDay.getFullYear(), weekDay.getMonth(), weekDay.getDate() - (weekDay.getDay() == 0 ? 6 : (weekDay.getDay() - 1)));
                            endDate = new Date(weekDay.getFullYear(), weekDay.getMonth(), weekDay.getDate() - (weekDay.getDay() == 0 ? 0 : (weekDay.getDay() - 7)));
                            rangeInput.val(fDate(startDate) + ' ' + options.rangeSplitter + ' ' + fDate(endDate));
                            selectCurrentWeek();
                        } else if (specificMonth){
                            startDate = new Date(year, month, 1);
                            endDate = new Date(year, month, Date.getDaysInMonth(year, month));
                            rangeInput.val(fDate(startDate) + ' ' + options.rangeSplitter + ' ' + fDate(endDate));
                        }else
	...
	function showRP(){
		if(rp.data('state') == 'closed'){ 
			...
			rp.fadeIn(300);
                        ...
                        rp.parent().css("top", rangeInput.offset().top + 21);
			options.onOpen(); 
		}
	}
	function hideRP(){
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			rp.find('.range-end').restoreDateFromData().hide();
                        rp.find('.week-picker').restoreDateFromData().hide();
                        rp.find('.month-picker').restoreDateFromData().hide();
			...
			setTimeout(function(){doneBtn.fadeIn();}, 400);
		}else if(jQuery(this).is('.ui-daterangepicker-specificWeek')){
                    doneBtn.hide();
                    rpPickers.show();
                    rp.find('.title-start').text("选择周");
                    rp.find('.range-start').restoreDateFromData().hide();
                    rp.find('.range-end').restoreDateFromData().hide();
                    rp.find('.month-picker').restoreDateFromData().hide();
                    rp.find('.week-picker').restoreDateFromData().show();
                    setTimeout(function(){doneBtn.fadeIn();}, 400);
                }else if(jQuery(this).is('.ui-daterangepicker-specificMonth')){
                    doneBtn.hide();
                    rpPickers.show();
                    rp.find('.title-start').text("选择月");
                    rp.find('.range-start').restoreDateFromData().hide();
                    rp.find('.range-end').restoreDateFromData().hide();
                    rp.find('.week-picker').restoreDateFromData().hide();
                    rp.find('.month-picker').restoreDateFromData().show();
                    setTimeout(function(){doneBtn.fadeIn();}, 400);
                }else {
                    //custom date range
                    doneBtn.hide();
	...
	//picker divs
	var rpPickers = jQuery('<div class="ranges ui-widget-content ui-corner-all ui-helper-clearfix"><div class="week-picker"><span class="title-start">Start Date</span></div><div class="range-start"><span class="title-start">Start Date</span></div><div class="range-end"><span class="title-end">End Date</span></div><div class="month-picker"><span class="title-start">Start Date</span></div></div>').appendTo(rp);
        var monthPickOptions = jQuery.extend(datepickerOptions, {changeMonth: true, changeYear: true});
	rpPickers.find('.range-start, .range-end, .week-picker').datepicker(options.datepickerOptions);
        rpPickers.find('.month-picker').datepicker(monthPickOptions);
