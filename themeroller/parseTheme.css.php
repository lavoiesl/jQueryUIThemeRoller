...
/*layout*/
body{
<?php echo texture_bg("bgColorLayout","bgTextureLayout","bgImgOpacityLayout")?> 
}
...
function _callback($match){
               ...
               $markup = join(',',$out); 
                $markup =  '<'.'?php echo texture_bg('.$markup.')?'.'>';
                return $markup;
}
...
foreach($themes as $key=>$value){
        ...
        $content = preg_replace_callback('/background[^;]*url\([^\)]*\)[^;]*;/i',_callback,$content);
//echo $content;
*/
?>
